...
public class Astar {
    ...
    public class AStarPathFinder implements PathFinder {
         ...
         */
        public AStarPathFinder(Tile[][] map, int maxSearchDistance, ) {
            this(map, maxSearchDistance, new ManhattanHeuristic());
        }
         ...
         * @param maxSearchDistance The maximum depth we'll search before giving up
         */
        public AStarPathFinder(Tile[][] map, int maxSearchDistance, AStarHeuristic heuristic) {
        ...
        public boolean isTileBlocked(int tx, int ty){
            return (map[tx][ty].getGameObject() != null);
        }
        ...
        protected boolean isValidLocation(Mover mover, int sx, int sy, int x, int y) {
            boolean invalid = (x < 0) || (y < 0) || (x >= map[0].length) || (y >= map.length));

            if ((!invalid) && ((sx != x) || (sy != y))) {
                invalid = isTileBlocked(x, y);
            }
